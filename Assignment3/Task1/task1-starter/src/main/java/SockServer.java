...
public class SockServer {
            ...
            while (true) {
                ...
                while (connected) {
                    ...
                    if (req.getString("type").equals("echo")) {
                        ...
                        res = concat(req);
                        //=========== CODE FROM PART C STARTS ======
                    } else if (req.getString("type").equals("currencyconverter")) {
                        res = currencyConverter(req);
                        //=========== CODE FROM PART C ENDS ======
    ...
    static JSONObject currencyConverter(JSONObject req) {
        System.out.println("Currency conversion request: " + req.toString());
        JSONObject res = testField(req, "from");
        if (!res.getBoolean("ok")) return res;

        res = testField(req, "to");
        if (!res.getBoolean("ok")) return res;

        res = testField(req, "amount");
        if (!res.getBoolean("ok")) return res;

        JSONObject response = new JSONObject();
        response.put("ok", true);
        response.put("type", "currencyconverter");
        ...
        Map<String, Double> ratesUSD = new HashMap<>();
        ratesUSD.put("USD", 1.0);
        ratesUSD.put("EUR", 0.93);
        ratesUSD.put("GBP", 0.82);

        if (!ratesUSD.containsKey(from) || !ratesUSD.containsKey(to)) {
            response.put("ok", false);
            response.put("message", "Unsupported currency code: " + (!ratesUSD.containsKey(from) ? from : to));
            return response;
        }

        double usdAmount = amount / ratesUSD.get(from); // convert to USD
        double converted = usdAmount * ratesUSD.get(to);

        response.put("converted", Math.round(converted * 100.0) / 100.0); // round to 2 decimals
        return response;
    }
    //=========== CODE FROM PART C ENDS ======
    ...
    // SENDING OUT RESPONSES, CLOSING CONNECTION
